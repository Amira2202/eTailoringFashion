...
<head>
  ...
  <style>
    ...
    .background-container {
      ...
      width: 100%;
      height: 100%;
      background-image: url('ubg4.jpg');
      background-size: cover;
      background-position: center;
      filter: blur(10px); /* Adjust blur strength as needed */
      z-index: 0; /* Send the blurred background to the bottom */
    }
...
<div class="container">
  <div class="row">
			<h1>cart</h1><hr>
			<a href='chudiviewcart.php'>View Cart</a>
			<?php 
			if(isset($_POST["addCart"])){
				if(isset($_SESSION["cart"]))
				{
					...
					if(!in_array($_GET["id"],$pid_array))
					{
						...
						header("location:chudiviewcart.php");
					}
					else
					{
			...
			$sql="select * from chudidesigns where pid='{$_GET["id"]}'";
			$res=$con->query($sql);
			if($res->num_rows>0)
			{
